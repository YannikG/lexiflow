...
Stored as TOML in the app config directory. Includes native language, `active_target_language`, onboarding flags, Ollama endpoint (optional), optional **Hugging Face token**, reader font size, **Theme** (`system` | `light` | `dark`), and the **data root** pointer.
