...
**Installers** — macOS disk image, Windows x64 installer package, Windows ARM64 installer package, Linux AppImage (x86_64). CI publishes all four on version tags. Unsigned in v1.
...
**Text metadata** — Structured record per text: identifier, title, **group** (display name), source and native language, optional source URL, timestamps, list of available **variants**, and `autogenerate_title` when the library title should be derived once from the **native variant** H1 after **markdown cleanup** (then cleared). No level field on text. At create: empty **add text dialog** title → provisional `Untitled` with autogenerate; user-entered title is kept. **Markdown cleanup** (or ensure-native on Target-tab paste) sets the library title from the native variant H1 when autogenerate is on, then clears the flag. **Plain translation** does not change the library title. Source URL can be entered in **add text dialog** or added later in **reader edit mode**.

**Document title** — Each **variant** has a title in that variant's language, usually as the markdown H1. **Text metadata** `title` is the **library title** for **sidebar** and reader header: user-provided at add-text, or derived from the native variant H1 after cleanup when autogenerate is on. Variant markdown keeps its own H1 in the body; translated H1 does not retitle the text. User edits the library title in **reader edit mode** (not from markdown H1 alone). Full **properties panel** editing is a later phase.
