...
**Consequences:** `lexiflow-core` owns `JobService` and queue schema; `lexiflow-ui` spawns and supervises worker lifecycle; embedded LLM inference runs in an isolated child process inside the worker so native ML crashes do not take down the UI process; CI tests core queue headless and UI worker supervision with fakes.
