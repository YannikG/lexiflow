...
LexiFlow runs as **two processes** from one install: a **UI process** (PySide6 only, no heavy ML imports) and a **worker process** (queue consumer, HTTP LLM client, embeddings). Jobs persist in `queue.sqlite`; processes coordinate via sqlite plus Qt local socket events. One installer launches both. If the worker crashes, the UI stays open and offers **Restart worker**; queue state is preserved.
...
**Consequences:** `lexiflow-core` owns `JobService` and queue schema; `lexiflow-ui` spawns and supervises worker lifecycle and (on the native path) a separate `llama-server` process so LLM crashes do not take down the UI; CI tests core queue headless and UI worker supervision with fakes.
...
**Worker idle shutdown:** Worker spawns lazily on first AI job. After ~5 min with no running or queued jobs, worker exits and unloads embedding state (same idle window as model sleep). Next job respawns worker. Native LLM process lifecycle is UI-owned ([ADR 0007](0007-native-llama-server-llm.md)).
