...
4. **Bundled llama-server** — CI downloads pinned llama.cpp prebuilts into `packaging/bin/<platform>/` (executable plus platform shared libraries: `.dylib`, `.so`, `.dll`); spec copies them to `bin/` in the bundle; `llama_server_binary()` resolves `{sys._MEIPASS}/bin/llama-server` before PATH.
