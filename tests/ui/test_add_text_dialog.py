...
def test_add_text_dialog_returns_title_and_default_group(qtbot, tmp_path: Path) -> None:
    dialog = AddTextDialog(
        data_root=tmp_path / "LexiFlow",
        target_language="es",
    )
    ...
    assert data.group == DEFAULT_TEXT_GROUP


def test_add_text_dialog_has_no_group_field(qtbot, tmp_path: Path) -> None:
    dialog = AddTextDialog(
        data_root=tmp_path / "LexiFlow",
        target_language="es",
    )
    qtbot.addWidget(dialog)
    dialog.show()
    assert dialog.findChild(QComboBox, "add_text_group") is None
...
def test_add_text_dialog_treats_blank_source_url_as_missing(
    ...
    assert title is not None and source_url is not None
    title.setText("Article title")
    source_url.setText("   ")
    dialog.paste_edit().setPlainText("Body text")
